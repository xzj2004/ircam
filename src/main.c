...
static const char* html_style = "body{font-family:Arial,sans-serif;margin:20px;background-color:#f0f0f0}"
    ".container{max-width:800px;margin:0 auto;background-color:white;padding:20px;border-radius:10px;box-shadow:0 0 10px rgba(0,0,0,0.1)}"
    "h1{color:#333;text-align:center}"
    "#thermalCanvas{width:auto;height:auto;max-width:100%;max-height:calc(100vh - 150px);margin:10px auto;display:block;background-color:#000}"
    ".controls{text-align:center;margin:10px 0}"
    ".controls button,.controls select{padding:8px 15px;margin:0 5px;font-size:14px;cursor:pointer;background-color:#4CAF50;color:white;border:none;border-radius:5px}"
    ".controls button:hover,.controls select:hover{background-color:#45a049}"
    ".info{text-align:center;margin-top:20px}"
    "#tempRange{margin:10px 0;text-align:center}";
static const char* html_head_end = "</style></head><body><div class=\"container\">";
static const char* html_body = "<h1>MLX90640 热成像相机</h1>"
    ...
    "</div>"
    "<div class=\"info\">"
    "<p>帧率: <span id=\"frameRate\">0</span> FPS</p>"
    "</div>";
static const char* html_script = "<script>"
    ...
    "const containerHeight=window.innerHeight - document.querySelector('h1').offsetHeight - document.querySelector('.controls').offsetHeight - document.querySelector('#tempRange').offsetHeight - document.querySelector('.info').offsetHeight - 70;" // 减去其他元素高度和一些边距
    "const aspectRatio=srcWidth/srcHeight;"
    "if(containerWidth/containerHeight > aspectRatio){"
    "dstHeight=Math.floor(containerHeight);"
    "dstWidth=Math.floor(dstHeight*aspectRatio);"
    "}else{"
    "dstWidth=Math.floor(containerWidth);"
    ...
    "document.addEventListener('DOMContentLoaded', () => {"
        "document.getElementById('resolution').value = '3';" // 设置默认值为3倍
        "changeResolution();"
    "});"
    "setInterval(()=>{"
